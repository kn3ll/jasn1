...
public class BerClassWriter {
  ...
  private BerObjectIdentifier parseObjectIdentfierValue(String name, AsnModule module)
      throws IOException {
  ...
  private void replaceParamtersByAnyTypes(
  ...
  private void writeChoiceDecodeFunction(
    ...
    for (int j = 0; j < componentTypes.size(); j++) {
      ...
      if (componentTag != null) {
        ...
        if (isDirectAnyOrChoice(componentType)) {
          write(
              getName(componentType) + " = new " + getClassNameOfComponent(componentType) + "();");
        ...
        if (componentTag != null) {
          if (isExplicit(componentTag)) {
            write(
                elseString
                    + "if (berTag.equals("
                    + getBerTagParametersString(componentTag)
                    + ")) {");
          } else {
            write(
                elseString
  ...
  private void writeSequenceDecodeIndefiniteLenghtPart(List<AsnElementType> componentTypes)
      throws IOException {
